...
class PayPalController {
  private frontendBaseUrl = 'https://angularwebstore.vercel.app';
  ...
  async completeOrder(req: Request, res: Response) {
    ...
    try {
      ...
      if (!token) {
        ...
        return res.redirect(`${this.frontendBaseUrl}/checkout/cancel`);
      }

      await paypalService.capturePayment(token);

      return res.redirect(`${this.frontendBaseUrl}/checkout/success?token=${token}`);
    } catch (error) {
      if (error instanceof ZodError) {
        return res.status(400).json({ success: false, message: "Validation error", issues: error.issues });
      }
